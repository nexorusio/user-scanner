...
def validate_naturephotographers(user: str) -> Result:
    encoded_user = urllib.parse.quote(user)
    url = f"https://community.naturephotographers.network/u/{encoded_user}.json"
    show_url = f"https://community.naturephotographers.network/u/{encoded_user}"
    headers = {
    ...
    def process(response) -> Result:
        if response.status_code == 404:
            try:
                data = response.json()
                if data.get("error_type") == "not_found" or any(
                    "not be found" in str(err).lower() for err in data.get("errors", [])
                ):
                ...
                if user_obj and isinstance(user_obj, dict) and user_obj.get("username"):
                    ...
                    if avatar_template:
                        ...
                        if avatar_url.startswith("/"):
                            avatar_url = f"https://community.naturephotographers.network{avatar_url}"
